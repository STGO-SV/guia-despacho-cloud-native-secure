...
public class CursosClient {
    ...
    public CursosClient(RestClient.Builder builder, @Value("${app.services.cursos-url}") String url) {
        this.client = builder.baseUrl(url).build();
    }
    public ResponseEntity<String> listar(String authorization) {
        return client.get().uri("/api/cursos").headers(h -> bearer(h, authorization))
                .retrieve().toEntity(String.class);
    }
    public ResponseEntity<String> crear(String json, String authorization) {
        return client.post().uri("/api/cursos").headers(h -> bearer(h, authorization))
